...
class Student(User):
    ...
    def RemoveCourse(self): 
      #  print("Remove Course was Successfully Used")
        ...
        if cursor.fetchone():
         #       cursor.execute("SELECT * FROM ENROLLMENT WHERE course_id = ? AND student_id = ?",(u_id,self.ID))
                cursor.execute("SELECT COURSE.* FROM ENROLLMENT, COURSE WHERE ENROLLMENT.course_id = ? AND ENROLLMENT.student_id = ? AND ENROLLMENT.course_id = COURSE.CRN",(u_id,self.ID))
                query_result = cursor.fetchall()
                for i in query_result:
                    print(i)
...
class Instructor(User):
    ...
    def Assemble(self):
      #  print("Print Schedule was Successfully Used")
        ...
        if y_n == "Y":    
            cursor.execute("""INSERT INTO ENROLLMENT (student_id, course_id) VALUES ('%s', '%s');""" % (u_id, u_class))
            conn.commit()
...
class Admin(User):
    ...
    def link_unlink(self):
        ...
        if option == 1:
            ...
            if option == 1:
                ...
                if cursor.fetchone():
                    ...
                    for i in query_result:
                      print(i)
                print("Courses:\n")
                cursor.execute("SELECT COURSE.* FROM COURSE JOIN ENROLLMENT ON COURSE.CRN = ENROLLMENT.course_id WHERE ENROLLMENT.student_id = ?",(s_id,))
                query_result = cursor.fetchall()
                for i in query_result:
                      print(i)
                course_id = self.get_integer("Enter COURSE ID: ","ID Must be an Integer")
                cursor.execute("SELECT * FROM ENROLLMENT WHERE instructor_id = ? AND course_id = ?",(i_id,course_id))
                if cursor.fetchone():
            ...
            if option == 2:
                s_id = self.get_integer("Enter Student ID: ","ID Must be an Integer")
                cursor.execute("SELECT * FROM STUDENT WHERE ID = ?",(s_id,))
                if cursor.fetchone():
                    cursor.execute("SELECT * FROM STUDENT WHERE ID = ?",(s_id,))
                    query_result = cursor.fetchall()
                    for i in query_result:
                    ...
                    for i in query_result:
                      print(i)
                course_id = self.get_integer("Enter COURSE ID: ","ID Must be an Integer")
                cursor.execute("SELECT * FROM ENROLLMENT WHERE student_id = ? AND course_id = ?",(s_id,course_id))
                if cursor.fetchone():
                    cursor.execute("UPDATE ENROLLMENT SET student_id = ? WHERE student_id = ? AND course_id = ?",(0000,s_id,course_id))
                    conn.commit()
                    print(f"Student was successfully unlinked with course (CRN: {course_id})")
                else:
                    print(f"Student is not linked to course {course_id}") 
        if option == 2:
            # Link
            option = int(input("Select Type of User:\n 1. Instructor\n 2. Student\n"))
            if option == 1:
                ...
                cursor.execute("SELECT * FROM ENROLLMENT WHERE instructor_id = ? AND course_id = ?",(i_id,course_id))
                if cursor.fetchone():
                    print("Instructor is already linked to the course\n")
                else:
                    cursor.execute("""INSERT INTO ENROLLMENT VALUES('%s','%s','%s');""" % (0,course_id,i_id))
                    print(f"Course ({course_id}) was successfully linked to instructor ({i_id})")
                    conn.commit()
            if option == 2:
                ...
                cursor.execute("SELECT * FROM ENROLLMENT WHERE student_id = ? AND course_id = ?",(s_id,course_id))
                if cursor.fetchone():
                    print("Student is already linked to the course\n")
                else:
                  #  cursor.execute("""INSERT INTO ENROLLMENT VALUES('%s','%s',('SELECT ENROLLMENT.instructor_id FROM ENROLLMENT WHERE instructor_id != 0 AND course_id = ?' (course_id,));""" % (s_id,course_id))
                    cursor.execute("""INSERT INTO ENROLLMENT VALUES('%s','%s','%s');""" % (s_id,course_id,0))
                    print(f"Course ({course_id}) was successfully linked to student ({s_id})")
                    conn.commit()
    def add_user(self):
        #WIP 
         option = int(input("Select Type of User:\n 1. Student\n 2. Instructor\n"))
